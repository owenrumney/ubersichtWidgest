  ...
  render() {
    ...
    if (Object.keys(info).length === 0) {
      if (cfg.displayDespiteNothing) {
        return <SpotifyDiv>Nothing Playing</SpotifyDiv>;
      } else {
        return <SpotifyDiv></SpotifyDiv>;
      }
    }
    ...
    if (cfg.displaySong.length > 0) {
      ...
      song = <SongSpan>{songName}</SongSpan>;
    }
    ...
    if (cfg.displayArtist.length > 0) {
      let ArtistSpan = styled("span")`
        ...
        padding: 0 ${cfg.padding};
      `;
      artist = <ArtistSpan>{info["artist"]}</ArtistSpan>;
    }
    ...
    if (cfg.displayTime === SpotifyStyle.TimeDisplay.TEXT) {
      let TimeSpan = styled("span")`
        ...
        padding: 0 ${cfg.padding};
      `;
      let duration = `${~~((totalTime % 3600) / 60)}:${
        ~~totalTime % 60 < 10 ? "0" : ""
      }${~~totalTime % 60}`;
      if (cfg.invertTime) {
        currentTime = totalTime - currentTime;
        let remaining = `-${~~((currentTime % 3600) / 60)}:${
          ~~currentTime % 60 < 10 ? "0" : ""
        }${~~currentTime % 60}`;
      ...
      time = (
        <TimeSVG viewBox="0 0 100 100" stroke={cfg.timeFill} strokeWidth="25">
          <circle cx="50" cy="50" r="30" fill="none" stroke={cfg.timeColor} />
          <circle
            cx="50"
            cy="50"
            r="35"
    ...
    let artwork;
    if (
      cfg.displayArtwork !== SpotifyStyle.ArtDisplay.NONE &&
      info["artwork"] !== "null"
    ) {
      let ArtworkImg = styled("img")`
        height: ${cfg.displayArtwork === SpotifyStyle.ArtDisplay.SHOW
          ? "80"
          : "75"}%;
        vertical-align: top;
        padding: 0 ${cfg.padding};
        ${cfg.displayArtwork === SpotifyStyle.ArtDisplay.SHOW
          ? ""
          : "border: 1px solid white;"}
      `;

      artwork = <ArtworkImg src={info["artwork"]}></ArtworkImg>;
    }
    ...
    for (let item of cfg.displayOrder) {
      switch (item) {
        case "I":
          ...
          let clonedArtwork = React.cloneElement(artwork, { key: index });
        ...
        case "S":
          ...
          let clonedSong = React.cloneElement(song, { key: index });
        ...
        case "A":
          ...
          let clonedArtist = React.cloneElement(artist, { key: index });
        ...
        case "T":
          ...
          let clonedTime = React.cloneElement(time, { key: index });
        ...
        case "s":
          ...
          let clonedShuffle = React.cloneElement(shuffle, { key: index });
        ...
        case "r":
          ...
          let clonedRepeat = React.cloneElement(repeat, { key: index });
        ...
        default:
          let TextSpan = styled("span")`
            padding: 0 ${cfg.padding};
          `;
          components.push(<TextSpan key={index}>{item}</TextSpan>);
          break;
      }
      index++;
    }

    return <SpotifyDiv>{components}</SpotifyDiv>;
  }
}
